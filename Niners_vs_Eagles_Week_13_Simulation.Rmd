---
title: "Niners Vs Eagles Week 13 Simulation (2023)"
output: pdf_document
---
...
## Finding Win Probability for Each Team as well as the Likelihood of Overtime

```{r}
eagles_win_prob <- mean(SimulationResults$eagles_win)
print(paste("Eagles Win Probability: ", round(eagles_win_prob,3)*100, "%"))
niners_win_prob <- mean(SimulationResults$niners_win)
print(paste("49ers Win Probability: ", round(niners_win_prob,3)*100, "%"))
overtime <- mean(SimulationResults$overtime)
print(paste("Overtime Probability: ",round(overtime,3)*100, "%"))
```

## Finding Expected Scored by Both Sides

```{r}
eagles_expected_points <- mean(as.numeric(SimulationResults$eagles_points))
print(paste("Eagles Expected Points: ", round(eagles_expected_points,0)))
niners_expected_points <- mean(as.numeric(SimulationResults$niners_points))
print(paste("Niners Expected Points: ", round(niners_expected_points,0)))
```

## Finding Out Which Players had the Most 2 Receiving TD Games
...
X2plus_TD <- bind_cols(brown_2plus_Td,smith_2plus_Td,swift_2plus_Td,stoll_2plus_Td,zacc_2plus_Td,jones_2plus_Td,watkins_2plus_Td,mccaffrey_2plus_Td,kittle_2plus_Td,              samuel_2plus_Td,aiyuk_2plus_Td,juszczyk_2plus_Td,bell_2plus_Td,dwelley_2plus_Td,jennings_2plus_Td,mccloud_2Plus_Td,mitchell_2Plus_Td)
```
...
SimulationResults %>% ggplot(aes(as.numeric(niners_passing_td))) +
  ...
  ggtitle("Niners Passing TDs Bar Chart") +
  theme_clean()
```

# Eagles Passing TDs Bar Chart
...
SimulationResults %>% ggplot(aes(as.numeric(eagles_passing_td))) +
  ...
  ggtitle("Eagles Passing TDs Bar Chart") +
  theme_clean()
```

## Niners Rushing TDs Chart
...
SimulationResults %>% ggplot(aes(as.numeric(niners_rushing_td))) +
  ...
  ggtitle("Niners Rushing TDs Bar Chart") +
  theme_clean()
```
...
SimulationResults %>% ggplot(aes(as.numeric(eagles_rushing_td))) +
  ...
  ggtitle("Eagles Rushing TDs Bar Chart") +
  theme_clean()
```

## AJ Brown had the most expected 2 Rec TD Games
